...
如果我使用的不是垃圾数据AAAAA，反而是精确的构造了一个地址覆盖，让他返回到我的恶意代码上
...
// 堆块的管理头部，相当于一个附加头信息
typedef struct header { // 定义一个header头结构体
    size_t size;           // 记录本块的用户数据区大小，用户实际能用的大小有多少
    bool is_free;          // 标记本块是否空闲
    struct header* next;   // 指向链表中的下一个管理头部，下一个块在哪里
} Header;
// 因为每个块都要包含这些信息
...
// 链表的头部，初始为NULL，相当于还没有块，还没安排给你用的内容
// 就和停车场一样，刚建好，还没开始记录信息
static Header* list_head = NULL;

void* my_malloc(size_t size) { // 分配的函数
    if (list_head == NULL) { // 检查是否是第一次，是第一次就初始化
        list_head = (Header*)heap; // 在堆最开始划出一块空间
        list_head->size = HEAP_SIZE - sizeof(Header); // 记录实际可用大小，因为头也占据空间
        list_head->is_free = true; // 标记此区域空闲
        list_head->next = NULL; // 下一个块地址NULL，只有这一个块
    }

    Header *current = list_head; // 临时记录current，从记录的第一个list_head开始查找
    while (current != NULL) { // 是否为空
        if (current->is_free && current->size >= size) { // 查看是否空闲且是否符合用户需要的大小
            // 找到！标记为已使用
            current->is_free = false;
            
            // 不能把头数据给用户，所以要+1跳过头信息的数据，返回实际的数据地址
            return (void*)(current + 1);
        }
        current = current->next; // 若这个块不合适，那就查看下一个
    }
